...
public:
  FilterDialog(insitu::Filter *parent_) { parent = parent_; }
...
protected:
  ...
  float getFloatSetting(std::string key) {
    // settings_mutex.lock();
  ...
  int getIntSetting(std::string key) {
    // settings_mutex.lock();
  ...
  std::string getStringSetting(std::string key) {
    // settings_mutex.lock();
...
public:
  ...
  */
  virtual bool hasSettingEditor(void) { return false; }

  /*
      called by Insitu, do not reimplement
  */
  virtual void openSettingEditor(void) { settingsDialog->open(); }
