...
const cv::Mat Stats::apply (void)
{
    ...
    cv::Scalar textColor = cv::Scalar(
            settings.get("r", 0).asInt(),
            settings.get("g", 0).asInt(),
            settings.get("b", 0).asInt(),
            settings.get("a", 255).asInt()
    );
    cv::Scalar bgColor = cv::Scalar(
            settings.get("r", 0).asInt(),
            settings.get("g", 0).asInt(),
            settings.get("b", 0).asInt(),
            0
    ...
    */
    ...
    if (settings.get("showCPU", true).asBool()) {
        stats.push_back("CPU: " + std::to_string(readCPUPercent()).substr(0,5) + "%");
    }
    if (settings.get("showMem", true).asBool()) {
        stats.push_back("Mem: " + std::to_string(readMemPercent()).substr(0,5) + "%");
    }
    if (settings.get("showDisk", true).asBool()) {
        stats.push_back("Disk: " + std::to_string(readDiskPercent()).substr(0,5) + "%");
    }
...
float Stats::readCPUPercent(void)
{
    ...
    if (stat.is_open()) {
        ...
        std::istringstream(cpu) >> discard >> user >> nice >> system >> idle >> iowait >> irq >> softirq;
